...
using LockthreatCompliance.Test.Base.TestData;
...
namespace LockthreatCompliance.Test.Base
{
    ...
    /// Provides methods to easily work with <see cref="LockthreatComplianceDbContext"/>.
    /// </summary>
    public abstract class AppTestBase<T> : AbpIntegratedTestBase<T> where T : AbpModule
    {
        ...
        {
            void NormalizeDbContext(LockthreatComplianceDbContext context)
            {
        ...
        protected void UsingDbContext(Action<LockthreatComplianceDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<LockthreatComplianceDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected TResult UsingDbContext<TResult>(Func<LockthreatComplianceDbContext, TResult> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<TResult> UsingDbContextAsync<TResult>(Func<LockthreatComplianceDbContext, Task<TResult>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<LockthreatComplianceDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<LockthreatComplianceDbContext>())
                {
                    action(context);
                    context.SaveChanges();
                }
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<LockthreatComplianceDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<LockthreatComplianceDbContext>())
        ...
        protected TResult UsingDbContext<TResult>(int? tenantId, Func<LockthreatComplianceDbContext, TResult> func)
        {
            TResult result;
        ...
        protected async Task<TResult> UsingDbContextAsync<TResult>(int? tenantId, Func<LockthreatComplianceDbContext, Task<TResult>> func)
        {
            TResult result;
