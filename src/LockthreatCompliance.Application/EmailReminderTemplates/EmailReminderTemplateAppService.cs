...
{
    public class EmailReminderTemplateAppService : LockthreatComplianceAppServiceBase, IEmailReminderTemplateAppService
    {
        private readonly IRepository<EmailReminderTemplate, long> _emailReminderRepository;
        ...
        public EmailReminderTemplateAppService(IRepository<EmailReminderTemplate, long> emailReminderRepository,
        ...
        public async Task<List<EmailReminderTemplateListDto>> GetAll()
        {

            var emailTemplate = new List<EmailReminderTemplateListDto>();
            try
            {

                var filteredtemplateTypes = _emailReminderRepository.GetAll().Include(x => x.WorkFlowPage).Include(x => x.AuditStatus);

                emailTemplate = (from obj in filteredtemplateTypes
                                 select new EmailReminderTemplateListDto()
                                 {
                                     Id = obj.Id,
                                     Days = obj.Days,
        ...
        public async Task<CreatorEditEmailReminderTemplateDto> GetEmailReminderTemplateForEdit(EntityDto input)
        {
            var query = new CreatorEditEmailReminderTemplateDto();

            query = await _emailReminderRepository.GetAll().Include(x => x.WorkFlowPage).Include(x => x.AuditStatus).Where(x => x.Id == input.Id).Select(x => new CreatorEditEmailReminderTemplateDto
            {
                Id = x.Id,
                Days = x.Days,
        ...
        public async Task<CreatorEditEmailReminderTemplateDto> GetStatusWies(int AuditStatusId)
        {

            var query = new CreatorEditEmailReminderTemplateDto();

            query = await _emailReminderRepository.GetAll().Include(x => x.WorkFlowPage).Include(x => x.AuditStatus).Where(x => x.AuditStatusId == AuditStatusId).Select(x => new CreatorEditEmailReminderTemplateDto
            {
                Id = x.Id,
                EmailBody = x.EmailBody,
                To = x.To,
        ...
        public async Task<long> CreateOrEdit(CreatorEditEmailReminderTemplateDto input)
        {
        ...
        protected virtual async Task<long> Create(CreatorEditEmailReminderTemplateDto input)
        {
            var getcheck = await _emailReminderRepository.GetAll().Where(x => x.WorkFlowPageId == input.WorkFlowPageId).FirstOrDefaultAsync();
            if (getcheck == null)
            {

                var emailTemplate = ObjectMapper.Map<EmailReminderTemplate>(input);
                if (AbpSession.TenantId != null)
                {
                    emailTemplate.TenantId = (int?)AbpSession.TenantId;
                }
              var id =   await _emailReminderRepository.InsertAndGetIdAsync(emailTemplate);
                return id;
            }
            else
            {
                var getcheckauditProject = await _emailReminderRepository.GetAll().Where(x => x.WorkFlowPageId == input.WorkFlowPageId && x.AuditStatusId == input.AuditStatusId).FirstOrDefaultAsync();
        ...
        protected virtual async Task<long> Update(CreatorEditEmailReminderTemplateDto input)
        {

            var emailTemplate = await _emailReminderRepository.GetAll().Where(p => p.WorkFlowPageId == input.WorkFlowPageId && p.AuditStatusId == input.AuditStatusId && p.Id == input.Id).FirstOrDefaultAsync();
            if (emailTemplate != null)
            {
               ...
               var id =  _emailReminderRepository.InsertOrUpdateAndGetId(emailTemplate);
                return id;
            ...
            else
            {
                ...
                {
                    var checkpageId = await _emailReminderRepository.GetAll().Where(x => x.WorkFlowPageId == input.WorkFlowPageId && x.Id == input.Id).FirstOrDefaultAsync();
                    if (checkpageId != null)
                    {
                        var emailTemplates = await _emailReminderRepository.GetAll().Where(x => x.Id == input.Id).FirstOrDefaultAsync();
                    ...
                    {
                        var checkworkflowId = await _emailReminderRepository.GetAll().Where(x => x.WorkFlowPageId == input.WorkFlowPageId).FirstOrDefaultAsync();
        ...
        public async Task Delete(EntityDto input)
        {
            try
            {
                await _emailReminderRepository.DeleteAsync(input.Id);
            }
            catch (Exception)
            {
                throw;
