...
{
    public class AccountAppService : LockthreatComplianceAppServiceBase, IAccountAppService
    {
        ...
        public Task<int?> ResolveAuditProjectId(ResolveTenantIdInput input)
        {
            if (string.IsNullOrEmpty(input.c))
            {
                return Task.FromResult(AbpSession.TenantId);
            ...
            if (query["auditProjectId"]==null)
            {
                return Task.FromResult<int?>(null);
            }

             var tenantId = Convert.ToInt32(query["auditProjectId"]) as int?;
            return Task.FromResult(tenantId);
        }



        public Task<int?> ResolveTTXEntityResponse (ResolveTenantIdInput input)
        {
            if (string.IsNullOrEmpty(input.c))
            {
                return Task.FromResult(AbpSession.TenantId);
            ...
            var tenantId = Convert.ToInt32(query["tteId"]) as int?;
            return Task.FromResult(tenantId);
        }

        public Task<int?> ResolveCertificatId (ResolveTenantIdInput input)
        {
            ...
            if (query["auditCertificateId"] == null)
            {
                return Task.FromResult<int?>(null);
            }

            var tenantId = Convert.ToInt32(query["auditCertificateId"]) as int?;
            return Task.FromResult(tenantId);
        }
        ...
        public async Task<ResetPasswordOutput> ResetPassword(ResetPasswordInput input)
        {
           ...
           // user.EncryptPassword = SimpleStringCipher.Instance.Encrypt(input.Password);
