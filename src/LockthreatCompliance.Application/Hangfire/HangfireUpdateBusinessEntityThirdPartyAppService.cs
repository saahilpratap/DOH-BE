...
{
    public class HangfireUpdateBusinessEntityThirdPartyAppService : LockthreatComplianceAppServiceBase, IHangfireUpdateBusinessEntityThirdPartyAppService
    {
        ...
        public HangfireUpdateBusinessEntityThirdPartyAppService(IRepository<User, long> userRepository, IRepository<BusinessEntityThirdParty> businessEntityThirdPartyRepository,
        ...
        public async Task UpdateThirdPartyBusinessEntity()
        {
            try
            {
            ...
            <soc:REQUESTTYPE>Update</soc:REQUESTTYPE>
            <soc:REFERENCEKEY> </soc:REFERENCEKEY>
            <soc:PAGENUMBER></soc:PAGENUMBER>
           </soc:FacilitiesOwnersGet>
         </soapenv:Body>
   </soapenv:Envelope>");
                ...
                using (WebResponse responses = requestGet.GetResponse())
                {
                    ...
                    foreach (var item in BusinessEntityThirdParty)
                    {
                        ...
                        if (check == null)
                        {
                            ...
                            if (item.DIRECTOR_INCHARGE_ENGLISH_NAME != "" && item.DIRECTOR_INCHARGE_ENGLISH_NAME != null && item.FACILITY_NAME != "")
                            {
                            ...
                            if (checkEntity == null)
                            {
                                ...
                                var checkavailable = checkBusinessentity.Where(x => x.LicenseNumber.Trim().ToLower() == item.LICENSE_NO.Trim().ToString().ToLower()).FirstOrDefault();
                                if (checkavailable == null)
                                {
                                    ...
                                    var checkUser = getuserCheck.Where(x => x.EmailAddress.Trim().ToLower() == AdminEmails.Trim().ToLower()).FirstOrDefault();
                                    ...
                                    if (checkUser == null)
                                    {
                                        ...
                                        user.Roles.Add(new UserRole(item.TenantId, user.Id, role.Id));
                        ...
                        else
                        {
                            ...
                            var businessEntity = _objectMapper.Map<BusinessEntityThirdParty>(item);
                            await _businessEntityThirdPartyRepository.UpdateAsync(businessEntity);
                            var checkEntity = await _businessEntityRepository.FirstOrDefaultAsync(x => x.LicenseNumber.Trim().ToLower() == item.LICENSE_NO.Trim().ToLower());
                            ...
                            if (checkEntity != null)
                            {
                                ...
                                if (checkavailable != null)
                                {
                                    ...
                                    _objectMapper.Map(businessentity, checkavailable);
                                    ...
                                    var orgid = _userOrganizationUnitRepository.GetAll().Where(x => x.Id == checkavailable.OrganizationUnitId).FirstOrDefault();
                                    ...
                                    await _userOrganizationUnitRepository.UpdateAsync(userOrganizationUnits);
                                }
                                else
                                {
