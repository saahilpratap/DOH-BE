...
{
   public class AuditProjectReminderHangFireAppservice: LockthreatComplianceAppServiceBase, IAuditProjectReminderHangFireAppservice
    {
        ...
        public async Task SendReaminderToAuditProjectByStatus()
        {
           try
            {
                ...
                if (auditstauss != null)
                {
                    var checkAuditStaus = auditstauss.Result.Where(x => x.Name.Trim().ToLower() == LockthreatComplianceConsts.Stage_1_Completed_AND_Audit_findingsReported.Trim().ToLower() && x.Name.Trim().ToLower()== LockthreatComplianceConsts.Stage_2_Completed_AND_Audit_findingsReported.Trim().ToLower()).Select(x=>x.Id).ToList();

                    var stage1staus = auditstauss.Result.Where(x => x.Name.Trim().ToLower() == LockthreatComplianceConsts.Stage_1_Completed_AND_Audit_findingsReported.Trim().ToLower()).FirstOrDefault();

                    var stage2staus = auditstauss.Result.Where(x => x.Name.Trim().ToLower() == LockthreatComplianceConsts.Stage_1_Completed_AND_Audit_findingsReported.Trim().ToLower()).FirstOrDefault();

                    var checkAuditProject = await _auditProjectRepository.GetAll().Where(x => checkAuditStaus.Contains((int)x.AuditStatusId)).Select(x=>x.Id).ToListAsync();
                    ...
                    var getpage = _workflowpageRepository.GetAll().Where(x => x.PageName.ToLower().Trim() == LockthreatComplianceConsts.Auditpage.Trim().ToLower()).FirstOrDefault();
                        ...
                        getother.AddRange((IEnumerable<DynamicParameterValue>)checkAuditStaus);
                        ...
                        checkAuditProject.ForEach(obj =>
                        {
                            ...
                            if (getauditProjects.LastModificationTime != null)
                            {
                                ...
                                var getReminderdays  =  _emailReminderTemplateRepository.GetAll().Where(x => x.AuditStatusId == getcheckAuidtProject.AuditStatusId && x.WorkFlowPageId == getpage.Id).FirstOrDefault();
                                ...
                                if (CheckDate.ToShortDateString() == DateTime.UtcNow.ToShortDateString())
                                {
                                    ...
                                    foreach (var item in getcheck)
                                    {
                                        ...
                                        if (getauditProject.AuditStatusId == stage1staus.Id)
                                        {
