...
{
    public class ImportFacilitySubTypesValueToExcelJob : BackgroundJob<ImportUsersFromExcelJobArgs>, ITransientDependency
    {
        ...
        private readonly IFacilitySubTypesListExcelDataReader _facilitySubTypesListExcelDataReader;
        private readonly IUnitOfWorkManager _unitOfWorkManager;

        public ImportFacilitySubTypesValueToExcelJob(
           ...
           IFacilitySubTypesListExcelDataReader facilitySubTypesListExcelDataReader,
           IUnitOfWorkManager unitOfWorkManager)
        {
            _facilityTypeRepository = facilityTypeRepository;
            _facilitySubTypeRepository = facilitySubTypeRepository;
            _facilitySubTypesListExcelDataReader = facilitySubTypesListExcelDataReader;
        ...
        public override void Execute(ImportUsersFromExcelJobArgs args)
        {
            ...
            {
                var entities = GetFacilitySubTypesFromExcelOrNull(args);
                ...
                CreateFacilitySubTypes(args, entities);
            }
        }

        private List<ImportFacilitySubType> GetFacilitySubTypesFromExcelOrNull(ImportUsersFromExcelJobArgs args)
        {
            try
            {
                var file = AsyncHelper.RunSync(() => _binaryObjectManager.GetOrNullAsync(args.BinaryObjectId));
                return _facilitySubTypesListExcelDataReader.GetFacilitySubTypesFromExcel(file.Bytes, args.TenantId);
            }
            catch (Exception)
            {
                return null;
        ...
        private void CreateFacilitySubTypes(ImportUsersFromExcelJobArgs args, List<ImportFacilitySubType> entities)
        {
            foreach (var item in entities)
            {
                var checkFacilityTypes = _facilityTypeRepository.GetAll().Where(x => x.Name.Trim().ToLower() == item.FacilityTypeName.Trim().ToLower()).FirstOrDefault();
                if (checkFacilityTypes != null)
                {
                    bool checkFacilitySubTypes = _facilitySubTypeRepository.GetAll().Any(p => p.FacilitySubTypeName.Trim().ToLower() == item.FacilitySubTypeName.Trim().ToLower() && p.FacilityTypeId == checkFacilityTypes.Id);
                    if (!checkFacilitySubTypes)
                    {
                        var facilitySubType = new FacilitySubType();
                        facilitySubType.TenantId = args.TenantId;
                        facilitySubType.FacilitySubTypeName = item.FacilitySubTypeName;
                        facilitySubType.FacilityTypeId = checkFacilityTypes.Id;
                        if (item.ControlType.ToLower().Trim() == "Basic".ToLower())
                        {
                            facilitySubType.ControlType = ControlType.Basic;
                        }
                        else if (item.ControlType.ToLower().Trim() == "Advanced".ToLower())
                        {
                            facilitySubType.ControlType = ControlType.Advanced;
                        }
                        else if (item.ControlType.ToLower().Trim() == "Transitional".ToLower())
                        {
                            facilitySubType.ControlType = ControlType.Transitional;
                        }
                        else
                        {
                            facilitySubType.ControlType = checkFacilityTypes.ControlType;
                        }
                        _facilitySubTypeRepository.Insert(facilitySubType);
                    }
                }
        ...
        private async Task ProcessImportUsersResultAsync(ImportUsersFromExcelJobArgs args)
        {
            if (args.User != null)
            {
                await _appNotifier.GlobalMessageAsync(
                    args.User,
                    "All Facility Sub Type Entities Import Done- " + args.Code, "FacilityType-" + args.User.UserId, //_localizationSource.GetString("AllUsersSuccessfullyImportedFromExcel"),
