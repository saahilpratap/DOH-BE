...
using LockthreatCompliance.MultiTenancy.Demo;
...
namespace LockthreatCompliance.MultiTenancy
{
    ...
    public class TenantManager : AbpTenantManager<Tenant, User>
    {
        ...
        public TenantManager(
            ...
            IPasswordHasher<User> passwordHasher, OrganizationUnitManager organizationUnitManager,
            IRepository<SubscribableEdition> subscribableEditionRepository, IRepository<EntityApplicationSetting> settingRepository,
            IRepository<OrganizationUnit, long> organizationUnitRepository, IRepository<Role> roleRepository) : base(
                tenantRepository,
            ...
            string emailActivationLink)
        {
            ...
            {
                throw new UserFriendlyException(LocalizationManager.GetString(LockthreatComplianceConsts.LocalizationSourceName, "TrialWithoutEndDateErrorMessage"));
            }
            ...
            using (var uow = _unitOfWorkManager.Begin(TransactionScopeOption.RequiresNew))
            {
                ...
                using (_unitOfWorkManager.Current.SetTenantId(tenant.Id))
                {
                    ...
                    await _roleRepository.InsertAsync(new Role { TenantId = tenant.Id, Name = StaticRoleNames.Tenants.BusinessEntity.Admin, DisplayName = StaticRoleNames.Tenants.BusinessEntity.Admin, IsStatic = true });
                    await _roleRepository.InsertAsync(new Role { TenantId = tenant.Id, Name = StaticRoleNames.Tenants.BusinessEntity.Manager, DisplayName = StaticRoleNames.Tenants.BusinessEntity.Manager, IsStatic = true });
                    await _roleRepository.InsertAsync(new Role { TenantId = tenant.Id, Name = StaticRoleNames.Tenants.BusinessEntity.User, DisplayName = StaticRoleNames.Tenants.BusinessEntity.User, IsStatic = true });
                    await _roleRepository.InsertAsync(new Role { TenantId = tenant.Id, Name = StaticRoleNames.Tenants.ExternalAudit.Admin, DisplayName = StaticRoleNames.Tenants.ExternalAudit.Admin, IsStatic = true });
                    await _roleRepository.InsertAsync(new Role { TenantId = tenant.Id, Name = StaticRoleNames.Tenants.ExternalAudit.Manager, DisplayName = StaticRoleNames.Tenants.ExternalAudit.Manager, IsStatic = true });
                    await _roleRepository.InsertAsync(new Role { TenantId = tenant.Id, Name = StaticRoleNames.Tenants.ExternalAudit.User, DisplayName = StaticRoleNames.Tenants.ExternalAudit.User, IsStatic = true });
                    ...
                    var runitext = _organizationUnitRepository.GetAll().Any(o => o.DisplayName == "Department of Health - Abu Dhabi" && o.TenantId == newTenantId);
                    if (!runitext)
                    {
                        var organizationUnit = new OrganizationUnit(newTenantId, "Department of Health - Abu Dhabi", null);
                        await _organizationUnitManager.CreateAsync(organizationUnit);
                        await _unitOfWorkManager.Current.SaveChangesAsync();
                    }

                    var hunitext = _organizationUnitRepository.GetAll().Any(o => o.DisplayName == "Healthcare Entities" && o.TenantId == newTenantId);
                    if (!hunitext)
                    {
                        var organizationUnit = new OrganizationUnit(newTenantId, "Healthcare Entities", null);
                        await _organizationUnitManager.CreateAsync(organizationUnit);
                        await _unitOfWorkManager.Current.SaveChangesAsync();
                    }

                    var eunitext = _organizationUnitRepository.GetAll().Any(o => o.DisplayName == "External Auditors" && o.TenantId == newTenantId);
                    if (!eunitext)
                    {
                        var organizationUnit = new OrganizationUnit(newTenantId, "External Auditors", null);
                        await _organizationUnitManager.CreateAsync(organizationUnit);
                        await _unitOfWorkManager.Current.SaveChangesAsync();
                    }

                    await _settingRepository.InsertAndGetIdAsync(new EntityApplicationSetting() { TenantId = tenant.Id, RootUnit = "Department of Health - Abu Dhabi", FirstUnit = "Healthcare Entities", SecondUnit = "External Auditors" });
                }
        ...
        public async Task CheckEditionAsync(int? editionId, bool isInTrialPeriod)
        {
            ...
            var error = LocalizationManager.GetSource(LockthreatComplianceConsts.LocalizationSourceName).GetString("FreeEditionsCannotHaveTrialVersions");
