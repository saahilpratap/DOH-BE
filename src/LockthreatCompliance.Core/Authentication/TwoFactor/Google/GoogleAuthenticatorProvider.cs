...
namespace LockthreatCompliance.Authentication.TwoFactor.Google
{
    public class GoogleAuthenticatorProvider : LockthreatComplianceServiceBase, IUserTwoFactorTokenProvider<User>, ITransientDependency
    {
        ...
        public Task<string> GenerateAsync(string purpose, UserManager<User> userManager, User user)
        {
            ...
            var setupInfo = _googleTwoFactorAuthenticateService.GenerateSetupCode("LockthreatCompliance", user.EmailAddress, user.GoogleAuthenticatorKey, 300, 300);
