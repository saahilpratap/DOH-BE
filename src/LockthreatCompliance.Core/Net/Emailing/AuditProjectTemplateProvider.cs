...
{
   public class AuditProjectTemplateProvider: IAuditProjectTemplateProvider, ISingletonDependency
    {
        private readonly IWebUrlService _webUrlService;
        private readonly ConcurrentDictionary<string, string> _defaultTemplates;

        public AuditProjectTemplateProvider(IWebUrlService webUrlService)
        {
        ...
        public string GetDefaultTemplate(int? tenantId)
        {
            ...
            var stream = typeof(EmailTemplateProvider).GetAssembly().GetManifestResourceStream("LockthreatCompliance.Net.Emailing.EmailTemplates.AuditTemplate.html");
            return _defaultTemplates.GetOrAdd(tenancyKey, key =>
            {
                using (var stream = typeof(EmailTemplateProvider).GetAssembly().GetManifestResourceStream("LockthreatCompliance.Net.Emailing.EmailTemplates.AuditTemplate.html"))
                {
