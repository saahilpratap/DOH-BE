...
using LockthreatCompliance.Authentication.TwoFactor.Google;
...
using LockthreatCompliance.Web.Models.TokenAuth;
using LockthreatCompliance.Authorization.Impersonation;
...
using LockthreatCompliance.Net.Sms;
using LockthreatCompliance.Notifications;
using LockthreatCompliance.Security.Recaptcha;
using LockthreatCompliance.Web.Authentication.External;
...
namespace LockthreatCompliance.Web.Controllers
{
    ...
    public class TokenAuthController : LockthreatComplianceControllerBase
    {
        ...
        [HttpPost]
        public async Task<AuthenticateResultModel> ApiAuthenticate([FromBody] AuthenticateModel model)
        {
            if (UseCaptchaOnLogin())
            {
                await ValidateReCaptcha(model.CaptchaResponse);
        ...
        public async Task SendTwoFactorAuthCode([FromBody] SendTwoFactorAuthCodeModel model)
        {
            ...
            if (model.Provider != GoogleAuthenticatorProvider.Name)
            {
                ...
                {
                    await _userEmailer.SendTwoFactorVerificationCode(user, cacheItem.Code);
        ...
        private async Task<IEnumerable<Claim>> CreateJwtClaims(ClaimsIdentity identity, User user, TimeSpan? expiration = null, TokenType tokenType = TokenType.AccessToken)
        {
            ...
            claims.AddRange(new[]
            {
                ...
                new Claim(AppConsts.UserType, user.Type.ToString())
             });
